...
/// <summary>
/// Central factory for constructing binder/navigation related commands.
/// Encapsulates the lambda wiring so the view model stays slimmer and testable.
/// </summary>
public class CommandFactory
{
    ...
    {
        _nav = nav;
        _pagesPerBinder = pagesPerBinder;
        _orderedFaceCount = orderedFaceCount;
        _jumpBinderInput = jumpBinderInput;
        _jumpPageInput = jumpPageInput;
        _slotsPerPage = slotsPerPage;
        _orderedFacesAccessor = orderedFacesAccessor;
    }

    private bool TryParseJump(out int binder, out int page)
    {
        binder = page = 0;
        if (!int.TryParse(_jumpBinderInput(), out binder) || binder < 1) return false;
        var pagesPerBinder = _pagesPerBinder();
        if (!int.TryParse(_jumpPageInput(), out page) || page < 1 || page > pagesPerBinder) return false;
        return true;
    }
    ...
    public ICommand CreateNextSet() => new RelayCommand(_ => {
        var faces = _orderedFacesAccessor();
        if (_nav.CanJumpSet(true, faces.Count)) _nav.JumpSet(true, faces, _slotsPerPage(), c => c.Set);
    }, _ => _nav.CanJumpSet(true, _orderedFaceCount()));
    public ICommand CreatePrevSet() => new RelayCommand(_ => {
        var faces = _orderedFacesAccessor();
        if (_nav.CanJumpSet(false, faces.Count)) _nav.JumpSet(false, faces, _slotsPerPage(), c => c.Set);
    }, _ => _nav.CanJumpSet(false, _orderedFaceCount()));
}
